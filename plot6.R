...
NEI_mvla <- NEI[NEI$SCC %in% coal_sccs & NEI$fips == "06037", ]
...
mvlaEmissionsByYear <- ddply(NEI_mvla, c("year"), summarise,
                            total.Emissions=sum(Emissions),
                            median.Emissions=median(Emissions), 
                            number.Readings=length(Emissions) )
...
modelb <- lm(total.Emissions ~ year, mvbEmissionsByYear)
abline(modelb, lwd=2)
with(mvlaEmissionsByYear, plot(year,total.Emissions))
modella <- lm(total.Emissions ~ year, mvlaEmissionsByYear)
